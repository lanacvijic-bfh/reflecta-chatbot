...
export default function Chat({ onImportExportReady }: ChatProps = {}) {
  ...
  const askPlanner = useCallback(async (conv: Conversation) => {
    ...
    try {
      ...
      // Update phase if provided in response
      if (response.phase !== undefined) {
        const newPhase = response.phase;
        if (newPhase !== conv.phase) {
          console.log(`📊 [PHASE UPDATE] Aktualisiere Phase: ${conv.phase} → ${newPhase}`);
          setConversation((prev) => {
            const updated = { ...prev, phase: newPhase };
            console.log(`✅ [PHASE UPDATE] Conversation State aktualisiert: phase=${updated.phase}`);
            return updated;
          });
        } else {
          console.log(`📊 [PHASE UPDATE] Phase unverändert: ${conv.phase}`);
        }
      } else {
        console.log(`⚠️ [PHASE UPDATE] Keine Phase in Response gefunden. Response:`, response);
      }
  ...
  const handleShowHelpExplanation = useCallback(async (cardId?: string) => {
    ...
    // In Phase 2: Side Cards können wieder angezeigt werden (für Diskussionen)
    // Zeige die Karte direkt an - in Phase 2 kann der User sie sehen, wenn er Fragen hat
    const currentPhase = conversation.phase || 1;
    if (currentPhase === 2) {
      // In Phase 2: Zeige die Side Card an, wenn der User darauf klickt
      // (Die Karten können helfen, wenn der User Fragen zur Diskussion hat)
      const card = cards.find((c) => c.id === cardId);
      if (card) {
        console.log(`📋 Phase 2: Zeige Side Card für ${cardId}`);
        setSelectedCard(card);
        return;
      }
    }

    // Phase 1: Fallback auf lokale Erklärung (für Erklärungen)
    // In Phase 1: Wenn der User eine Rückfrage stellt, rufe Backend an für vereinfachte Erklärung
    const card = await loadCardById(cardId);
      ...
      >
        <div className="chat-inner-wrapper">
          ...
          {conversation.turns.map((turn, idx) => {
            // "Mehr Details" Buttons entfernt - werden nicht mehr angezeigt
          ...
          {(() => {
            ...
            // KRITISCH: Einstufungs-Buttons nur in Phase 1 anzeigen!
            // In Phase 2 wird diskutiert, nicht eingestuft
            if (conversation.phase === 2) {
              return null; // Keine Einstufungs-Buttons in Phase 2
            }
            
            // KRITISCH: Wenn ask_card mit card_id vorhanden ist, zeige Buttons IMMER an!
            // ask_card bedeutet automatisch, dass eine Frage gestellt wird - keine zusätzliche Frage-Erkennung nötig
            ...
            // Zeige Buttons IMMER an, wenn ask_card mit card_id vorhanden ist (nur in Phase 1)
            // Keine weitere Prüfung nötig - ask_card bedeutet automatisch eine gestellte Frage
